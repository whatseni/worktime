...
export const handleCreateUser = async ({username, userrole, userPhone, userBirth, bank, bankaccount, company}: any) => {
  try {
...
export const handleUpdateUser = async ({username, userrole, userPhone, userBirth, bank, bankaccount, company}: any) => {
  try {
...
export const handleGetAllUserTimeMonth = async ({ date, company }: any) => {
  try {
    const result = await axios.post("http://localhost:5000/time/get-alluser-time-month", {
      date: date,
      company: company
    })
  } catch (error) {
...
export const handleGetAllUserAllTimeMonth = async ({ date, company }: any) => {
  try {
    const result = await axios.post("http://localhost:5000/time/get-alluser-alltime-month", {
      date: date,
      company: company
    })
  } catch (error) {
